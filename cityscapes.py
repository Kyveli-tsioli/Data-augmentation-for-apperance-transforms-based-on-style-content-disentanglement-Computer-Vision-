...
class Cityscapes(data.Dataset):
    ...
    def collect_ids(self):
        im_dir = os.path.join(self.root, 'leftImg8bit_trainvaltest/leftImg8bit', self.split)
        ids = []
    ...
    def img_path(self, id):
        fmt = 'leftImg8bit_trainvaltest/leftImg8bit/{}/{}/{}_leftImg8bit.png'
        subdir = id.split('_')[0]
        path = fmt.format(self.split, subdir, id)
        return os.path.join(self.root, path)

    def label_path(self, id):
        fmt = 'gtFine_trainvaltest/gtFine/{}/{}/{}_gtFine_labelIds.png'
        subdir = id.split('_')[0]
        path = fmt.format(self.split, subdir, id)
        return os.path.join(self.root, path)
