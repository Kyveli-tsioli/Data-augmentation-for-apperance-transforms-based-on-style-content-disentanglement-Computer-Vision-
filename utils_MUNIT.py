...
from networks_MUNIT import Vgg16
from torch.autograd import Variable
from torch.optim import lr_scheduler
from torchvision import transforms
from data_MUNIT import ImageFilelist, ImageFolder
import torch
...
import time
#from datasets.gta5 import gta
from gta5_MUNIT import gta
#from datasets.cityscapes import cityscapes
from cityscapes_MUNIT import cityscapes
# Methods
...
def get_all_data_loaders_(conf, opts):
    batch_size = conf['batch_size']
    num_workers = conf['num_workers']
    if 'new_size' in conf:
        new_size_a = new_size_b = conf['new_size']
    ...
    dataset_a = 'gta'
    dataset_b = 'cityscapes'

    train_loader_a = get_data_loader_folder_(conf['data_root'], batch_size,
            new_size_a, height, width, num_workers, train=True, crop=True, flip=True, dataset=dataset_a)
    test_loader_a = get_data_loader_folder_(conf['data_root'], batch_size,
            new_size_a, height, width, num_workers, train=False, crop=True, flip=False, dataset=dataset_a)
    train_loader_b = get_data_loader_folder_(conf['data_root'], batch_size,
            new_size_b, height, width, num_workers, train=True, crop=True, flip=True, dataset=dataset_b, set=opts.cityscapes_train_set)
    test_loader_b = get_data_loader_folder_(conf['data_root'], batch_size,
            new_size_b, height, width, num_workers, train=False, crop=True, flip=False, dataset=dataset_b, set='val')
    return train_loader_a, train_loader_b, test_loader_a, test_loader_b

def get_data_loader_folder_(dataroot, batch_size, new_size=None,
                           height=256, width=256, num_workers=4, train=False, crop=False, flip=False, dataset=None, set=None):
    if dataset == 'gta':
        dataset = gta(dataroot, new_size, height, width, train_split=train, crop=crop, flip=flip)
    elif dataset == 'cityscapes':
        dataset = cityscapes(dataroot, new_size, height, width, train_split=train, crop=crop, flip=flip, set=set)
...
def print_opts(opt, config, nets, output_directory, training_a, training_b, test_a, test_b):
    args = vars(opt)
    # save to the disk
    ...
    with open(file_name, 'wt') as opt_file:
        ...
        for k, v in sorted(config.items()):
            opt_file.write('%s: %s\n' % (str(k), str(v)))
        opt_file.write('-------------- End ----------------\n')
        print('#training images a = %d' % training_a, file=opt_file)
        print('#training images b = %d' % training_b, file=opt_file)
        print('#test images a = %d' % test_a, file=opt_file)
        print('#test images b = %d' % test_b, file=opt_file)

        names = ['gen_a', 'gen_b', 'dis_a', 'dis_b', 'segmentor']
        for name in names:
