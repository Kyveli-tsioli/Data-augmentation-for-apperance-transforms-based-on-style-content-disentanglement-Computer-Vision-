...
def main(outdir, dataset, datadir, batch_size, lr, p_synthetic, step, iterations,
  ...
  #dataloader takes a dataset and returns batches of images and corresponding labels 
  #we can use batch size and suffle after each epoch, organize data in mini-batches and shuffle them with dataloader 
  #dataloader job: to sample mini-batches from a dataset
  ...
  #print("content_code_real size", content_code_real.size()) #torch.Size([1, 256, 128, 256]) dimension of downsampled image
  ...
  style_dim=8
  styles_real= torch.randn(display_size,style_dim, 1, 1) #try diaspora=10, experiment
  #styles_real= np.sqrt(10)*torch.randn(display_size,style_dim, 1, 1) #try diaspora=10, experiment
  #rint("styles_real", styles_real.size())
  ...
  #print("max", torch.max(synthetic)) #0.9996
  #print("min", torch.min(synthetic)) #-0.8750
  ...
  while True:
      ...
      for im1, im2, label in loader: 
          ...
          if (generate_prob < p_synthetic):
            ...
            #same picture, but original
            ##test_im2= im2.squeeze(0) 
            ##test_im2_tr= test_im2.numpy().transpose([1,2,0])  #(512, 1024, 3)
            ##test_im2_tr_rgb= (test_im2_tr +1) * 0.5 *255
            ##test_im2_tr_color = np.asarray(test_im2_tr_rgb, np.uint8)
            ##plt.figure()
            ##plt.imshow(test_im2_tr_color)
            ##plt.savefig("original_im2.png") 
          ...
          # load data/label
          #im1 = make_variable(im1, requires_grad=False) #use the im1 for the segmentation task
          #label = make_variable(label, requires_grad=False)
          

          # forward pass and compute loss
          #preds = net(im) #was before
          ...
          losses.append(loss.item()) #changed it, new pytorch version 

          #losses.append(loss.data[0]) was before
